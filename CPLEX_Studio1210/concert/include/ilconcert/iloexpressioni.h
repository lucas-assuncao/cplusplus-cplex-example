...
// PVilim (my understanding):
// Abstract base class for all kinds of generators. Generator is one index, one
// collection and a set of filters. Consider for example:
//     forall (i in 1..10: i > 5)
//  * i is an index (integer index in this case)
//  * 1..10 is a collection (integer range collection in this case)
//  * i > 5 is a filter
// There are descendant classes for different types of the index (integer,
// float, symbol, tuple) and collection. Therefore the base class has just filters.
// TODO/PV: Isn't it an opportunity for a template? E.g. IloInt could be template parameter.
...
// PVilim: This seems to be implementation of "if - then - else" of constraints
// where the if condition is evaluated by OPL (i.e. solver will see only the
// then part or else part). I.e. for example:
//     constraints {
//       if (1 > 0) then
//          x > 5;
//       else
