...
#ifdef ILO_WINDOWS
        // Poor man's spinlock. See the definition of GlobalMutex why we
        // don't use a mutex or similar.
...
private:
	IloInt       _max;    // max number of elements before adjusting _shift
	IloInt const _sizeof; // size of a single element
	IloInt       _size;   // current size
	IloInt       _mod;    // X&_mod is remainder of X/(2**_shift)
	IloInt       _shift;  // data is 2**_shift chunks of size 2**_shift
	char**       _data;   // storage
